...
func (h *TaskHandler) GetProgressUnified(c *gin.Context) {
	...
	// 计算进度百分比
	progressPercent := 0.0
	if totalSamples, ok := progressData["total_samples"].(float64); ok && totalSamples > 0 {
		if generatedCount, ok := progressData["generated_count"].(float64); ok {
			progressPercent = (generatedCount / totalSamples) * 100
		}
	}
