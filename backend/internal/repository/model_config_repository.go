...
// GetByModelPath 根据模型路径获取模型配置
func (r *ModelConfigRepository) GetByModelPath(modelPath string) (*models.ModelConfig, error) {
	var config models.ModelConfig
	err := r.db.Where("model_path = ?", modelPath).First(&config).Error
	if err != nil {
		return nil, err
	}
	return &config, nil
...
func (r *ModelConfigRepository) GetByName(name string) (*models.ModelConfig, error) {
	var config models.ModelConfig
